...
(function() {
  ...
  const QUESTS = [
    ...
    },
    {
      ...
      before: '她只會「回應你」。你沒 prompt 她就靜止。',
      after: 'Hook 是**你預先寫的程式**，掛在她工作的幾個關鍵時刻上，到那個時刻就自動跑 — 不是她「有意志」，是你設計的反射動作在她身上生效：<br>· 你開啟 Claude 瞬間（SessionStart）自動載入前一次 session 的待辦清單<br>· 你送出 prompt 瞬間（UserPromptSubmit）自動注入當下專案的 context<br>· 呼叫危險工具前（PreToolUse）自動要求 confirm<br>· 對話結束（Stop）自動把今天學到的東西寫進記憶<br><br>結果：下次見你不用重新解釋。你第一次覺得她的工作「有連貫性」。<br><br><em>注意：hook 只在 Claude 這類 session 事件上觸發，不是你關機 / 睡覺時。要「24/7 背景運作」請看 Lv.7 Annuli。</em><br><br><em>每個 CLI 事件名不同：Claude Code 最豐富；Gemini 用 BeforeTool / AfterTool；Codex 要啟用 feature flag；Copilot 用 camelCase。</em>',
      deliverable: '至少 2 個 hooks 運作中（例如 Stop + PreToolUse）',
    },
    {
      level: 5,
      zh: '通達者',
    ...
    },
    {
      ...
      before: '她只在你的電腦的 IDE / 終端機裡。離開電腦就斷線。',
